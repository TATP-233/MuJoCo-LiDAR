...
_MACOS_VIEWER_HELP = """\
On macOS, MuJoCo's viewer must run under mjpython (not plain python).
...
def _height_rgba(z_norm: np.ndarray) -> np.ndarray:
    """Map normalized height to RGBA without matplotlib."""
...
if __name__ == "__main__":
    ...
    with mujoco.viewer.launch_passive(mj_model, mj_data) as viewer:
        ...
        while viewer.is_running():
            ...
            rot = lidar.sensor_rotation
            pos = lidar.sensor_position
            world_points = points @ rot.T + pos

            z_values = world_points[:, 2]
            z_min, z_max = z_values.min(), z_values.max()
            z_norm = (z_values - z_min) / (z_max - z_min) if z_max > z_min else np.zeros_like(z_values)
